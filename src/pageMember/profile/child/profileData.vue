            ...
            <view class="list-box">
                ...
                <view class="list-value">
                    ...
                    <radio-group @change="changeRadio" v-else-if="item.selectedGender">
                        <radio value="男" :checked="item.value === '男'" name="gender" style="transform: scale(0.7)"
                            color="#00ca76" />男
                        <radio value="女" name="gender" :checked="item.value === '女'" style="transform: scale(0.7)"
                            color="#00ca76" />女
                    </radio-group>
                    <!-- 对于日期选择的特殊情况 -->
                    ...
                    <view class="" v-else-if="item.selectedCity">
                        <picker mode="multiSelector" @change="bindPickerChange" @columnchange="columnchange"
                            ...
                            <view class="uni-input">{{ item.value || cityActiveText }}</view>
                        </picker>
                    </view>
                </view>
            </view>
...
// 整理后的userdatas
const datas = ref([
    { key: "账号", value: "", id: "account", prompt: "请输入用户名", disabled: true },
...
// 省
const city = ref(CITYDATA.map((item) => { return item.name }));
// 市
const area = ref(CITYDATA[cityIndex.value[0]].children.map((item) => { return item.name }));
// 区
const county = ref(
    CITYDATA[cityIndex.value[0]].children[cityIndex.value[1]].children.map((item) => { return item.name })
);
...
const bindPickerChange = (e) => {
    ...
    cityCodeIndex.value[1].code = CITYDATA[cityIndex.value[0]].children[cityIndex.value[1]].code
    cityCodeIndex.value[2].code = CITYDATA[cityIndex.value[0]].children[cityIndex.value[1]].children[cityIndex.value[2]].code
...
const columnchange = (e) => {
    ...
    // 改变area的数组
    area.value = CITYDATA[cityIndex.value[0]].children.map((item) => { return item.name })
    // 改变county的数组
    county.value =
        CITYDATA[cityIndex.value[0]].children[cityIndex.value[1]].children.map((item) => { return item.name })
};
